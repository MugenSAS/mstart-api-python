...
class OSCMessage(object):
	...
	def append(self, argument, typehint=None):
		...
		if hasattr(argument, '__iter__') and type(argument) != str:
			for arg in argument:
		...
		self.typetags += tag                            
		#self.message += binary.decode()
		self.message += binary#MODIF
...
def OSCTimeTag(time):
	...
	if time > 0:
		...
		binary = struct.pack('>LL')#, 0L, 1L)#MODIF
...
def _readString(data):
	...
	"""
	##BEGIN MODIF
	## string.find doesn't work in Python3
	length = 0
...
def decodeOSC(data):
	...
	if address.startswith(b","):#(",")#MODIF
		typetags = address.decode()
		address = ""
	else:
		typetags = ""
	...
	if address == "#bundle":
		...
		if typetags.startswith(b","):#(",")#MODIF
			for tag in typetags[1:].decode():#typetags[1:]:#MODIF
...
class OSCStreamRequestHandler(StreamRequestHandler, OSCAddressSpace):
	...
	def _transmit(self, data):
		sent = 0
		while sent < len(data):
			print("OSC OSCStreamRequestHandler _transmit")
	...
	def _transmitMsg(self, msg):
		...
		"""
		print("OSC OSCStreamRequestHandler _transmitMsg")
		if not isinstance(msg, OSCMessage):
			raise TypeError("'msg' argument is not an OSCMessage or OSCBundle object")
